# `@bsv-poker/game-omaha`

Omaha / Omaha-8 (4 hole cards, use exactly 2); optional Hi-Lo split. A **pure** `GameModule` (no I/O, no time, no randomness) over the `@bsv-poker/engine`
...
```
node --test "packages/game-omaha/test/**/*.test.ts"
```
