...
Razz (lowball Stud; highest up-card brings in — reversed from Stud). A **pure** `GameModule` (no I/O, no time, no randomness) over the `@bsv-poker/engine`
...
```
node --test "packages/game-razz/test/**/*.test.ts"
```
