# `@bsv-poker/game-blackjack`

Blackjack module (card values, ace 11/1) — house-banked variant scaffolding. A **pure** `GameModule` (no I/O, no time, no randomness) over the `@bsv-poker/engine`
...
```
node --test "packages/game-blackjack/test/**/*.test.ts"
```
