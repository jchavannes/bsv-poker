# `@bsv-poker/game-stud` — Security model

Seven-card Stud (ante + bring-in; up-cards drive the action). It is a **pure deterministic** game module: no network, no I/O, no time, no randomness.
