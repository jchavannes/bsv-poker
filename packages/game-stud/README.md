...
Seven-card Stud (ante + bring-in; up-cards drive the action). A **pure** `GameModule` (no I/O, no time, no randomness) over the `@bsv-poker/engine`
...
```
node --test "packages/game-stud/test/**/*.test.ts"
```
