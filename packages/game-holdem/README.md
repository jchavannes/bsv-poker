# `@bsv-poker/game-holdem`

Texas Hold'em (2 hole cards, 5 community); the Phase-1 flagship variant. A **pure** `GameModule` (no I/O, no time, no randomness) over the `@bsv-poker/engine`
...
```
node --test "packages/game-holdem/test/**/*.test.ts"
```
