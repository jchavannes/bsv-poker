/**
 * On-chain TABLE play E2E — proves that ACTUAL GAMEPLAY settles on-chain, for every variant. For
 * each variant we play a full hand on the real engine, read the engine-determined winner + pot, then
 * FUND that pot on-chain (coinbase → N-of-N multisig bound to the hand) and SETTLE it to the winner
 * as real confirmed transactions through the real node. No offline-only path: the hand's money moves
 * on-chain. (Chips are scaled to satoshis; regtest is the chain.)
 */

import { spawn, type ChildProcess } from 'node:child_process';
import assert from 'node:assert/strict';
import { RealBsvNode } from '@bsv-poker/adapters/real-node';
import { playOfflineHand, offlineRuleset, seededShuffle } from '@bsv-poker/app-services';
import {
  OP,
  genKeyPair,
  signPreimage,
  fairPlayCommitment,
  fundingLocking,
  fundingUnlocking,
  type Script,
  type KeyPair,
} from '@bsv-poker/script-templates-ts';
import { bytesToHex, type BranchBinding, type Variant } from '@bsv-poker/protocol-types';
import { type Tx, serializeTxWire, txidWire, sighashMessage, SIGHASH_ALL_FORKID } from '@bsv-poker/tx-builder';

const NODE_DIR = process.env.BSV_NODE_DIR ?? 'D:\\claude\\ACM 01\\bonded-subsat-channel';
const PORT = Number(process.env.BSV_NODE_PORT ?? 8744);
const SUBSIDY = 5_000_000_000;
const SCALE = 1_000_000; // 1 chip = 1e6 sats, so per-tx fees are negligible
const FEE = 1000;
const STACK = 100;
let daemon: ChildProcess | null = null;

const BIND: BranchBinding = { gid: 'a1'.repeat(8), rulesetHash: 'b2'.repeat(32), round: 0, stateHash: 'c3'.repeat(32), actingSeat: -1, successorCommitment: '00'.repeat(32) };
const p2pkh = (pub: Uint8Array): Script => [OP.OP_DUP, OP.OP_HASH160, fairPlayCommitment(pub), OP.OP_EQUALVERIFY, OP.OP_CHECKSIG];
const sigT = (msg: Uint8Array, k: KeyPair): Uint8Array => Uint8Array.from([...signPreimage(msg, k.priv), SIGHASH_ALL_FORKID]);

/** Play a hand; find a deal with a single net winner so there is a pot to settle on-chain. */
function playWithWinner(variant: Variant, seats: number): { winner: number; potChips: number } {
  const ruleset = offlineRuleset(variant, seats);
  const seatInits = Array.from({ length: seats }, (_, i) => ({ seat: i, stack: STACK }));
  for (let seed = 0; seed < 200; seed++) {
    const b = new Uint8Array(32);
    b[0] = seed & 0xff;
    b[2] = variant.charCodeAt(0);
    const st = playOfflineHand(variant, ruleset, seatInits, seededShuffle(b, 52));
    const nets = st.seats.map((s) => s.stack - STACK);
    const max = Math.max(...nets);
    if (max > 0 && nets.filter((n) => n === max).length === 1) {
      return { winner: nets.indexOf(max), potChips: max };
    }
  }
  throw new Error(`${variant} ${seats}p: no single-winner deal found`);
}

async function settleOnChain(node: RealBsvNode, variant: Variant, seats: number): Promise<void> {
  const { winner, potChips } = playWithWinner(variant, seats);
  const pot = potChips * SCALE;
  const players = Array.from({ length: seats }, () => genKeyPair());
  const funder = genKeyPair();

  const cb = await node.generateBlock(bytesToHex(funder.pubCompressed));

  // FUNDING: coinbase → [pot multisig over the seated players] + [change back to funder].
  const fundingScript = fundingLocking(BIND, players.map((p) => p.pubCompressed));
  const fundingTx: Tx = {
    version: 1,
    inputs: [{ prevTxid: cb.coinbaseTxid, vout: 0, sequence: 0xffffffff }],
    outputs: [
      { satoshis: pot, locking: fundingScript },
      { satoshis: SUBSIDY - pot - FEE, locking: p2pkh(funder.pubCompressed) },
    ],
    nLockTime: 0,
  };
  const fundSig: Script = [sigT(sighashMessage(fundingTx, 0, p2pkh(funder.pubCompressed), SUBSIDY), funder), funder.pubCompressed];
  assert.equal((await node.submitTx(bytesToHex(serializeTxWire(fundingTx, [fundSig])))).ok, true, `${variant} funding`);
  await node.generateBlock(bytesToHex(funder.pubCompressed));
  const fundingTxid = txidWire(fundingTx, [fundSig]);

  // SETTLEMENT: the N-of-N pot → the engine-determined winner.
  const payout = pot - FEE;
  const settleTx: Tx = { version: 1, inputs: [{ prevTxid: fundingTxid, vout: 0, sequence: 0xffffffff }], outputs: [{ satoshis: payout, locking: p2pkh(players[winner]!.pubCompressed) }], nLockTime: 0 };
  const msg = sighashMessage(settleTx, 0, fundingScript, pot);
  const settleSig = fundingUnlocking(players.map((p) => sigT(msg, p)));
  assert.equal((await node.submitTx(bytesToHex(serializeTxWire(settleTx, [settleSig])))).ok, true, `${variant} settlement`);
  await node.generateBlock(bytesToHex(funder.pubCompressed));

  const settleTxid = txidWire(settleTx, [settleSig]);
  assert.equal((await node.outpointStatus(fundingTxid, 0)).unspent, false, `${variant}: pot consumed`);
  const won = await node.outpointStatus(settleTxid, 0);
  assert.equal(won.unspent, true, `${variant}: winner output confirmed`);
  assert.equal(won.value, payout, `${variant}: winner receives the pot`);
  console.log(`[onchain-table] ${variant} ${seats}p: seat ${winner} won ${potChips} chips → on-chain pot ${pot} → settled ${won.value} (confirmed)`);
}

async function main(): Promise<void> {
  daemon = spawn('python', ['-m', 'channel.cli', 'daemon-start', '--port', String(PORT), '--db', ':memory:'], { cwd: NODE_DIR, env: { ...process.env, PYTHONPATH: 'src' }, stdio: 'ignore' });
  const node = new RealBsvNode('127.0.0.1', PORT);
  try {
    const dl = Date.now() + 30000;
    while (!(await node.ping().catch(() => false))) { if (Date.now() > dl) throw new Error('node down'); await new Promise((r) => setTimeout(r, 400)); }
    for (const [variant, seats] of [['holdem', 2], ['omaha', 3], ['stud', 4], ['draw', 2], ['razz', 3]] as const) {
      await settleOnChain(node, variant, seats);
    }
    console.log('\n[onchain-table] PASS — every variant: a real engine hand funded + settled ON-CHAIN through the real node (all play on-chain).');
  } finally {
    await node.shutdown();
    daemon?.kill();
  }
}

main().then(() => process.exit(0), (e) => { console.error('[onchain-table] FAIL:', (e as Error).message); daemon?.kill(); process.exit(1); });
