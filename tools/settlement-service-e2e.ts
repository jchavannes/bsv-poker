/**
 * Human-path settlement bridge E2E — SERVERLESS. A node-side settlement service (the player's OWN
 * local helper, holding the human's on-chain key) is driven over localhost HTTP exactly as the desktop
 * UI would: GET /pubkey, then POST /settle when the hand ends. The service is its own P2P node and
 * dials a peer in the table mesh; a second peer (the opponent) co-signs over the PEER-TO-PEER mesh; the
 * service submits the N-of-N settlement to the BSV node and it confirms on-chain. This is how the
 * browser-bound human client settles on-chain without ever signing in the webview — no relay server.
 */
...
async function main(): Promise<void> {
  ...
  // The opponent (player B) is a P2P node in THIS process; it also serves as the rendezvous peer the
  // spawned settlement-service dials. No relay server — just two peers on the mesh.
  const hub = new P2PTransport(0);
  await hub.start([]);
  procs.push(spawn('node', [join(ROOT, 'tools', 'settlement-service.ts'), '--port', String(SVC_PORT), '--node', String(NODE_PORT), '--peer', `127.0.0.1:${hub.boundPort()}`], { cwd: ROOT, stdio: 'ignore' }));
  ...
  try {
    ...
    // Wait until the settlement-service node has joined the mesh (so co-sign frames reach it).
    while (hub.peerCount() < 1) { if (Date.now() > dl) throw new Error('settlement-service did not join the mesh'); await new Promise((r) => setTimeout(r, 200)); }
    let pubA = '';
    ...
    console.log('[svc-e2e] UI → POST /settle on the node-side service; opponent co-signs over the P2P mesh…');
    const [svcResp] = await Promise.all([
      fetch(`http://127.0.0.1:${SVC_PORT}/settle`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ tableId, idx: 0, n: 2, fundingTxid, potValueSats: escrow, ocPubsHex, outputs, submit: true }) }).then((r) => r.json() as Promise<{ ok: boolean; txid: string | null }>),
      coSignSettlement({ relay: hub, tableId, idx: 1, myKey: keyB, settleTx: { version: 1, inputs: [{ prevTxid: fundingTxid, vout: 0, sequence: 0xffffffff }], outputs: outputs.map((o) => ({ satoshis: o.sats, locking: p2pkh(Uint8Array.from(Buffer.from(o.pubHex, 'hex'))) })) as TxOutput[], nLockTime: 0 }, fundingScript, potValue: escrow, n: 2, submit: false, submitTx: (raw) => node.submitTx(raw) }),
    ]);
    ...
    console.log('\n[svc-e2e] PASS — the browser-bound human client settled on-chain via its own node-side service, opponent co-signing over the P2P mesh (no signing in the webview, no relay server).');
  } finally {
