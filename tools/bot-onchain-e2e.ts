 ...
 * hand PEER-TO-PEER, then — at hand-end — co-sign the N-of-N settlement of the table escrow to the
 * FINAL stacks over the gossip channel and submit it to the real in-tree node. This is exactly what
 * the bot daemon does on each completed hand: play over the wire, then settle on-chain, no party
 * acting alone. There is NO relay server — each peer is its own P2P node.
 */
...
async function main(): Promise<void> {
  ...
  const mesh = await p2pMesh(2); // two peer nodes — no relay server
  ...
  try {
    ...
    // PLAY: two networked peers play a full hand peer-to-peer.
    console.log('[bot-onchain] two peers playing a hand peer-to-peer…');
    const seats = [{ seat: 0, stack: STACK }, { seat: 1, stack: STACK }];
    const mk = (seat: number, e: number) => new NetworkedTableClient({ relay: mesh.transports[seat]!, tableId, mySeat: seat, seats, ruleset: RULES, entropy: Uint8Array.from(Array.from({ length: 32 }, (_, i) => (i * (seat * 6 + 7) + e) % 251)) });
    ...
    console.log('[bot-onchain] peers co-signing the on-chain settlement over the gossip channel…');
    const results = await Promise.all(
      players.map((k, i) => coSignSettlement({ relay: mesh.transports[i]!, tableId, idx: i, myKey: k, settleTx, fundingScript, potValue: escrow, n: 2, submit: i === 0, submitTx: (raw) => node.submitTx(raw) })),
    );
    await node.generateBlock(bytesToHex(funder.pubCompressed));
    ...
    console.log('\n[bot-onchain] PASS — a live networked hand auto-settled ON-CHAIN at hand-end (play peer-to-peer → N-of-N co-sign over gossip → confirmed), no server.');
  } finally {
    await node.shutdown();
    mesh.close();
  }
...
main().then(() => process.exit(0), (e) => { console.error('[bot-onchain] FAIL:', (e as Error).message); process.exit(1); });
