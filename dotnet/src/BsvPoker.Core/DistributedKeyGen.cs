...
public static class DistributedKeyGen
{
    ...
    public static (ThresholdSharing.Polynomial Poly, Round1 Msg) Deal(int myIndex, byte[] myPub, int t, IReadOnlyDictionary<int, byte[]> peerPubs)
    {
        ...
        foreach (var (j, pubj) in peerPubs) sealedShares[j] = SealScalar(poly.Eval(j), pubj);
    ...
    public static (byte[] JointShare, byte[] JointPublicKey, int? Blame) Finalize(int myIndex, byte[] myPriv, IReadOnlyList<Round1> all)
    {
        ...
        foreach (var r in all)
        {
            ...
            byte[] fij;
            try { fij = OpenScalar(sealedHex, myPriv); }
    ...
    public static string SealScalar(byte[] scalar32, byte[] recipientPub33)
    {
        ...
        var aead = Aead.Seal(key, scalar32, recipientPub33);   // recipient pubkey bound as AAD
        return Convert.ToHexString(Concat(Concat(eph.Pub, nonce), aead)).ToLowerInvariant();
    }

    /// <summary>Open a scalar sealed by <see cref="SealScalar"/> with the recipient's private key (throws on
    /// wrong key / tamper).</summary>
    public static byte[] OpenScalar(string sealedHex, byte[] recipientPriv32)
    {
        ...
        return Aead.Open(key, aead, myPub);
    }
