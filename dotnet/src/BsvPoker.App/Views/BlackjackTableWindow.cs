...
public sealed class BlackjackTableWindow : Window
{
    ...
    private readonly Button _hit = Mk("Hit"), _stand = Mk("Stand"), _double = Mk("Double"), _leave = Mk("Leave table");
    private readonly TextBlock _bankInfo = new() { Foreground = new SolidColorBrush(Color.FromRgb(0xFF, 0xE0, 0x82)), FontSize = 14, FontWeight = FontWeights.SemiBold, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 6, 0, 0) };
    ...
    public BlackjackTableWindow(Window owner, NetBlackjack bj, Func<string, string?>? labelFor = null)
    {
        ...
        _leave.Background = new SolidColorBrush(Color.FromRgb(0x7A, 0x2E, 0x2E));
        _leave.Click += (_, _) => { if (_bj.SessionOver) { Close(); return; } _bj.Leave(); Refresh(); };
        btns.Children.Add(_hit); btns.Children.Add(_stand); btns.Children.Add(_double);
        root.Children.Add(btns);
        var leaveRow = new StackPanel { Orientation = Orientation.Horizontal, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 4, 0, 0) };
        leaveRow.Children.Add(_leave);
        root.Children.Add(leaveRow);
        root.Children.Add(_bankInfo);
    ...
    private void Refresh()
    {
        ...
        bool dealerOpen = _bj.State is NetBlackjack.Phase.DealerPlay or NetBlackjack.Phase.HandOver or NetBlackjack.Phase.Done;
        _dealerInfo.Text = dealer.Count > 0 ? (dealerOpen ? $"Dealer total: {Blackjack.Value(dealer).Total}" : "Dealer shows…") : "";
        ...
        // your turn — atomic: it's your turn, you're not done (incl. busted), and no drawn card is still pending
        bool myTurn = _bj.MyTurn;
        ...
        // over it becomes Close. Disabled only briefly while a leave is already pending.
        _leave.Content = _bj.SessionOver ? "Close" : "Leave table";
        _leave.IsEnabled = _bj.SessionOver || !_bj.Leaving;
