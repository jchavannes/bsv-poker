...
2. **Real money HAS moved end-to-end on regtest (consensus-proven).** `tools/RegtestE2E` runs against a
   ...
   pays the pot to the winner (mined) — both consensus-accepted. So the whole money spine — fund →
   SPV-verify → signed spend, and pot escrow → settlement — is proven against real BSV consensus rules.
